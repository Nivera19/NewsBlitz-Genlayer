...
export default function App() {
  ...
  const enterWaiting = useCallback(
    async (id: string, topicHint: string, host: boolean) => {
      setScreen("waiting");
  ...
  const refreshWaiting = useCallback(async (id: string, topicHint: string) => {
    try {
      ...
      const topicInfo =
        TOPIC_MAP[info.topic] || TOPIC_MAP.world;
  ...
  const handleSubmitAnswer = useCallback(
    async (answer: string) => {
      ...
      try {
        ...
        // Read on-chain result for feedback
        ...
        try {
          ...
          if (res.found) {
            ...
            setScore((s) => s + pts);
            setFeedback({
              type: res.correct ? "correct" : "wrong",
              message: res.correct ? "Correct!" : "Wrong answer!",
              points: pts,
            });
          } else {
        ...
        setFeedback({
          type: "wrong",
          message: "TX failed — " + (e instanceof Error ? e.message.slice(0, 40) : "error"),
          points: 0,
        });
      } finally {
        setSubmitting(false);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      if (isHost) await finishGame(roomId);
    } catch { /* ok */ }
    ...
    try {
      ...
      setLbTopic("Error loading leaderboard");
      console.error(e);
    }
  }, [isHost, roomId]);
  ...
  switch (screen) {
    ...
    case "waiting":
      return (
        <WaitingRoomScreen
          topic={topicTitle || topicInfo.label}
    ...
    case "game":
      return (
        <GameScreen
          ...
          timeLeft={timeLeft}
          totalTime={QUESTION_TIME_MS / 1000}
          score={score}
